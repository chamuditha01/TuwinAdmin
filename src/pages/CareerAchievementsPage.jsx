...
import {
  ...
  deleteCareerAchievement,
} from '../api/client';

const EMPTY_FORM = { Title: '', heading: '', description: '', footer: '' };

function CareerAchievementForm({ initial, onCancel, onSave }) {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div className="form-field">
        <label>Footer</label>
        <input value={form.footer} onChange={setField('footer')} />
      </div>
...
export default function CareerAchievementsPage() {
  ...
  const load = async () => {
    ...
    setError('');
    try {
      setEntries(await getCareerAchievements());
    } catch (err) {
      setError(err.message);
    } finally {
      setLoading(false);
  ...
  const handleAdd = async (form) => {
    await addCareerAchievement(form);
    setModalMode(null);
    await load();
  };

  const handleEdit = async (form) => {
    await updateCareerAchievement(modalMode._row, form);
    setModalMode(null);
    await load();
  };
  ...
  const handleDelete = async (row) => {
    ...
    setDeletingRow(row);
    try {
      await deleteCareerAchievement(row);
      await load();
    } catch (err) {
      setError(err.message);
    } finally {
    ...
    <div>
      ...
      <div className="card">
        {loading ? (
          ...
          <div className="empty-state">No achievements yet. Click "Add Achievement" to create one.</div>
        ) : (
            ...
            <tbody>
              {entries.map((e) => (
                <tr key={e._row}>
                  ...
                  <td data-label="Footer">{e.footer}</td>
      ...
      {modalMode === 'add' && (
        <Modal title="Add Achievement" onClose={() => setModalMode(null)}>
      ...
      {isEditing && (
        <Modal title="Edit Achievement" onClose={() => setModalMode(null)}>
          <CareerAchievementForm
            initial={modalMode}
            onCancel={() => setModalMode(null)}
            onSave={handleEdit}
